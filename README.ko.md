...
**EasySubway를 엔지니어링 맥락으로 삼아 배우는 라운드 기반 대중교통 경로 탐색 실습 가이드입니다.**
...
여기서 **RAPTOR는 대중교통 경로 탐색 알고리즘**을 뜻합니다. 이름이 비슷한 언어 모델용 검색 기법이 아닙니다. 이 저장소는 직접 작성한 학습 자료이며 EasySubway 운영 런타임의 포크가 아닙니다.

> **EasySubway 예제 읽는 법.** 코드 설명은 [2026년 9월 7일에 검토한 Backend 구현](https://github.com/AquilaXk/easysubway-backend/tree/1d80b7afc58bf788dd76846ea7dc86fcb8f1cfaa)을 기준으로 합니다. 뒷부분에서는 계획된 아키텍처도 학습 목표로 설명하지만, 모든 기능이 이미 배포되었다는 뜻은 아닙니다. 실행 가능한 시간표, 식별자, 해시 형태의 값은 모두 가상의 테스트 데이터입니다.
...
7. [EasySubway에서 RAPTOR의 위치](#7-easysubway에서-raptor의-위치)
...
EasySubway에서 중요한 질문은 “빠른 열차를 찾을 수 있는가?”보다 넓습니다. “서버가 설명 가능하고 접근성을 충족하며 식별 정보가 일관된 여정을 반환하거나, 여정을 지어내지 않고 실패할 수 있는가?”입니다.
...
완료된 이슈에 기술된 EasySubway 목표에서 운행 달력 예외, 운행일 경계를 넘는 조회 구간, 전날 운행편의 발생 시점, 정확한 배차 간격 의미, 실시간 정보 적용 가능성은 모두 입력 수용 검증과 질의 의미에 속합니다. 화면 표시 형식으로 고칠 수 있는 문제가 아닙니다. [운행일과 시간표](docs/03_service_days_and_timetables.md)를 참고하세요.
...
엄격한 계단 없는 이동 모드에서는 확인된 계단을 제외합니다. 근거가 없거나 검증되지 않았다면 실습의 보수적인 스냅샷 전체 정책에 따라 명시적인 오류 유형으로 입력 수용에 실패합니다. 이는 EasySubway의 더 풍부한 범위별 근거 모델보다 엄격하고 단순한 정책이며, 전체 모델을 구현한 것은 아닙니다.
...
## 7. EasySubway에서 RAPTOR의 위치

![EasySubway의 저장소별 책임과 변경 불가능한 경로 탐색 입력](assets/easysubway_architecture.jpg)

학습 목표는 [Hub #2742](https://github.com/AquilaXk/easysubway/issues/2742)와 [Hub #1393](https://github.com/AquilaXk/easysubway/issues/1393)에 기록된 5개 저장소의 책임 모델을 따릅니다.

| 담당 | 라우터가 받거나 제공해야 하는 것 |
|---|---|
| Data | 수용 검증을 통과한 불변 `server-route-bundle`, 별도의 지도·역 카탈로그 산출물 |
| Backend | 유일한 권위의 Journey 계산과 타입이 명시된 결과·실패 |
| Mobile | 생성된 계약 사용과 접근성을 갖춘 화면 표시, 로컬 대체 라우터는 제공하지 않음 |
| Platform | 소스 없는 배포, 불변 산출물 활성화, 기능 지원 여부를 반영한 준비 상태 확인 |
| Hub | 저장소 간 조율과 최종 근거·식별 정보 결정 |

검토한 구현의 [알고리즘 결정 기록](https://github.com/AquilaXk/easysubway-backend/blob/1d80b7afc58bf788dd76846ea7dc86fcb8f1cfaa/tools/routes/route-algorithm-v2-adr.json)은 단일 출발 RAPTOR를 활성 상태로, 프로파일 모드를 PR #312 대기 중인 비활성 상태로 표시합니다. 이는 코드가 선언한 기능이며 실제 배포 상태를 측정한 결과가 아닙니다.

[Backend 경로 계획기](https://github.com/AquilaXk/easysubway-backend/blob/1d80b7afc58bf788dd76846ea7dc86fcb8f1cfaa/backend/src/main/java/com/easysubway/route/application/service/RouteTimetableRaptorPlanner.java)에는 개선된 정류장 수집, 패턴에서 가장 앞선 표시 위치, 스캔 작업 공간, 명시적 접근 전이, 최종 EXIT 투영이 있습니다. 확인한 경로에서는 여전히 `SearchRouteV2Command`를 받습니다. [Backend #306](https://github.com/AquilaXk/easysubway-backend/issues/306)의 목표는 Journey 자체 실행에서 이 호환성 경계를 제거하는 것입니다.

현재 어댑터 형태나 고정 라벨 상수를 완성된 아키텍처로 보지 마세요. [전체 흐름을 다루는 장](docs/04_easysubway_end_to_end.md)은 main 코드에서 관찰한 내용과 완료된 이슈의 책임을 대응시키되, 둘이 같다고 주장하지 않습니다.
...
EasySubway의 [Backend #307](https://github.com/AquilaXk/easysubway-backend/issues/307)은 출발 시각, 목적지 도착 시각, 환승, 도보 부담, 접근성 부담, 연결 여유 시간을 포함하는 더 넓은 목표와 버전이 지정된 대표 경로·용량 동작을 정의합니다.
...
독립적인 [후보 집합 모듈](src/pareto.py)은 지배 관계, 목적 벡터 중복 제거, 용량 손실 시 실패를 보여줍니다. 모든 비지배 벡터를 유지하거나 `RAPTOR_FRONTIER_CAPACITY_EXCEEDED`를 발생시킵니다. 단일 라벨 노선 스캔을 완전한 McRAPTOR로 바꾸지는 않으며 EasySubway가 요구하는 모든 대표 태그도 구현하지 않습니다.
...
EasySubway의 [#305](https://github.com/AquilaXk/easysubway-backend/issues/305), [#308](https://github.com/AquilaXk/easysubway-backend/issues/308), [#309](https://github.com/AquilaXk/easysubway-backend/issues/309), [#310](https://github.com/AquilaXk/easysubway-backend/issues/310)이 정의하는 전체 목표는 더 넓습니다. 여기서는 작동 원리를 배우되, 전체 운영 계약을 구현한 것으로 받아들이지 마세요.
...
9개 장, 3개 노트북, 실행 가능한 예제, 범위를 좁힌 테스트를 연결한 학습 과정입니다. 각 구성 요소는 RAPTOR의 불변 조건이나 EasySubway 적용 경계를 설명합니다. 파일 개수 제한 없이 새 학습 자료를 추가할 수 있습니다.
...
| 서비스 연결 | [04 — EasySubway 전체 흐름](docs/04_easysubway_end_to_end.md) | main·이슈 출처 대응표 |
...
**설명하지만 구현했다고 주장하지 않는 것:** 완전한 Journey V3/Profile V1 통신 계약, 여러 운행일의 프로파일 병합, GTFS·배차 간격 데이터 수집, 전체 McRAPTOR 상태와 대표 태그, 서명된 산출물 검증, 실시간 제공자, 모바일 UI, K3s 활성화, 운임 계산, 배포 환경 성능 수용 검증.
...
이 저장소는 EasySubway 제품 저장소 5개와 분리해 유지합니다. 알고리즘과 주변 계약을 가르치는 자료이며, 새로운 데이터 제공 서비스나 또 다른 경로 계산 권위가 되어서는 안 됩니다.

[CI 워크플로](.github/workflows/ci.yml)는 Python 3.11.14와 3.14.6에서 전체 테스트, 예제 3개, 새 커널의 노트북 실행을 수행합니다. 저장소 읽기 전용 권한과 커밋에 고정된 액션을 사용합니다. 실행 전 메모리에서 노트북 출력을 지우므로 저장된 출력을 새로운 검증 근거로 취급하지 않습니다. JupyterLab UI 없이 검증하려면 `requirements-ci.txt`를 설치하세요. 최초 게시용 스크립트는 소스 트리 밖에 있습니다.

게시 스크립트의 기본 제안은 비공개 저장소, `main` 브랜치, 이슈·토론 활성화, 위키·프로젝트 비활성화, squash 병합만 허용, 병합된 브랜치 자동 정리, 관련 저장소 토픽입니다. main 브랜치 보호도 명시적으로 요청할 수 있습니다. 실제 원격 설정은 생성 후 다시 읽어 확인해야 하며, 스크립트가 있다는 이유만으로 적용되었다고 설명해서는 안 됩니다.
...
**프로젝트 맥락.** **2026년 9월 7일**에 검토한 EasySubway main과 연결된 이슈의 목표를 사용합니다. [04장](docs/04_easysubway_end_to_end.md#source-and-assumption-register)에 출처와 가정의 경계를 기록했습니다. 이슈 페이지는 바뀔 수 있으므로 Backend main 코드 관찰에는 불변 커밋 링크를 사용합니다.
