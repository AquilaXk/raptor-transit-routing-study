...
예제와 독립 테스트 실행기는 Python 표준 라이브러리만 필요합니다. 저장소 루트에서 시작하세요.
...
[CI 워크플로](.github/workflows/ci.yml)는 Python 3.11, 3.12, 3.13에서 전체 테스트, 예제 3개, 두 언어판 노트북 6개의 새 커널 실행을 수행합니다. 저장소 읽기 전용 권한과 커밋에 고정된 액션을 사용합니다. 실행 전 메모리에서 노트북 출력을 지우므로 저장된 출력을 새로운 검증 근거로 취급하지 않습니다. JupyterLab UI 없이 검증하려면 `requirements-ci.txt`를 설치하세요.
